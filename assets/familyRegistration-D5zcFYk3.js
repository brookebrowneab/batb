import{g as U}from"./index-D8JtWtNr.js";import{f as D,d as j,u as P,c as F}from"./students-C4Im7lf4.js";import{f as H,a as z,d as G}from"./contracts-N-dwWFje.js";import{g as M,u as O,a as V}from"./storage-BUQKYqIH.js";import{v as W,h as Y,r as J}from"./sanitizeHtml-D1SfNnaG.js";import{v as K,e as L}from"./registration-BJf__55_.js";import{i as Q}from"./emailValidation-DoK9MuSU.js";import{e as d}from"./escapeHtml-lW1fV86Q.js";import{f as X,b as Z,c as ee,s as te,d as ne}from"./rolePreferences-DCjwN29-.js";import{v as ae,i as se,s as q}from"./rolePreferences-B9XCX_X4.js";import{s as re}from"./notifications-D-qX7iYr.js";import"./purify.es-B9ZVCkUG.js";import"./scheduling-DSZ8OVmG.js";let E=[],t=null,S=[],h=null,g=1,A=null,C=[],k=[];function oe(){const e=window.location.hash||"",n=e.indexOf("?");return n===-1?null:new URLSearchParams(e.slice(n+1)).get("studentId")||null}function N(){return C.length>0&&se(A)}function T(){const e=["Student Info","Parent/Guardian","Audition Song","Photo"];return N()&&e.push("Role Preferences"),e.push("Review & Sign"),e}async function ie(e,n=null,a=!1){const[o,i,l,v]=await Promise.all([D(e),H(),X(),Z()]);if(E=o.data||[],h=i.data,A=l.data,C=v.data||[],a?t=null:n?t=E.find(f=>f.id===n)||null:E.length>0?t=E[0]:t=null,t){const[f,_]=await Promise.all([z(t.id),N()?ee(t.id):{data:[]}]);S=f.data||[],k=_.data||[]}else S=[],k=[]}function le(){const e=T();let n='<div class="wizard-progress">';for(let a=1;a<=e.length;a++){a>1&&(n+=`<div class="wizard-progress__connector${a<=g?" wizard-progress__connector--complete":""}"></div>`);let o="wizard-progress__dot",i=String(a);a<g?(o+=" wizard-progress__dot--complete",i="✓"):a===g&&(o+=" wizard-progress__dot--active"),n+=`<div class="wizard-progress__step"><div class="${o}">${i}</div><span class="${a===g?"wizard-progress__label wizard-progress__label--active":"wizard-progress__label"}">${e[a-1]}</span></div>`}return n+="</div>",n}function de(){let e="";if(E.length>1){const n=E.map(a=>{const o=t&&a.id===t.id?"selected":"";return`<option value="${a.id}" ${o}>${d(a.first_name||"Unnamed")} ${d(a.last_name||"Student")}</option>`}).join("");e+=`<div class="student-selector"><label for="student-select"><strong>Student:</strong></label><select id="student-select">${n}</select></div>`}return E.length>=1&&(e+=`
  ...
  `}function ue(){if(!t)return'<div class="card"><h2>Parent/Guardian Information</h2><p class="placeholder-notice">Complete student info first.</p></div>';const e=me();return`
    <div class="card">
  ...
  `}function ge(){if(!t)return'<div class="card"><h2>Audition Song</h2><p class="placeholder-notice">Complete student info first.</p></div>';const e=t,n=e.sings_own_disney_song||!1;return`
    <div class="card">
      <h2>Audition Song</h2>
  ...
  `}function fe(){if(!t)return'<div class="card"><h2>Student Photo</h2><p class="placeholder-notice">Complete student info first.</p></div>';const e=t.photo_storage_path;return`
    <div class="card">
      <h2>Student Photo</h2>
      <p style="font-size:var(--text-small);color:var(--color-text-secondary);margin-bottom:var(--space-md)">
  ...
  `}function ve(){if(!t)return'<div class="card"><h2>Role Preferences</h2><p class="placeholder-notice">Complete student info first.</p></div>';const e=q(k),n=new Map(e.map(l=>[l.audition_role_id,l.rank_order])),a=C.map((l,v)=>v+1);let o=C.map(l=>{const v=n.get(l.id)||"",f=a.map(_=>`<option value="${_}" ${v===_?"selected":""}>${_}</option>`).join("");return`
    ...
    <div class="card">
      ...
      <p style="font-size:var(--text-small);color:var(--color-text-secondary);margin-bottom:var(--space-md)">
        Rank the roles in order of the ones you'd most like to audition for.
        <br><br>
        Put #1 next to your top choice — this is the role you most want to audition for. Then continue ranking the rest in order of preference (#2, #3, etc.).
        <br><br>
        Be thoughtful and honest about what excites you most!
      </p>
  ...
  `}function be(){if(!t)return'<div class="card"><h2>Review & Sign</h2><p class="placeholder-notice">Complete previous steps first.</p></div>';const e=t,n=h&&Y(S,h.id);let a="";if(!h)a='<p class="placeholder-notice" style="margin-top:var(--space-md)">No contract available.</p>';else if(n){const o=S.find(i=>i.contract_id===h.id);a=`
    ...
    `}else a=`
      <div style="margin-top:var(--space-lg)">
        ...
        <div class="contract-text">${J(h.text_snapshot)}</div>
  ...
  `}async function I(e,n={}){const{forceSendScheduleEmail:a=!1}=n;if(!t)return;const o=L(t,S,(h==null?void 0:h.id)||null),i=t.registration_complete===!0;o.complete!==t.registration_complete&&(await P(t.id,{registration_complete:o.complete},e),t.registration_complete=o.complete);let l=null;if(o.complete&&(a||!i)){const{error:f}=await re(t.id);f&&(console.error("Registration schedule email failed:",f.message||f),l=f)}return{complete:o.complete,emailError:l}}function ye(e,n){const a=document.getElementById("student-select");a&&a.addEventListener("change",()=>{g=1,n(a.value)});const o=document.getElementById("add-student-btn");o&&o.addEventListener("click",()=>{t=null,S=[],g=1,n(null,!0)});const i=document.getElementById("clear-registration-btn");i&&i.addEventListener("click",async()=>{if(!(t!=null&&t.id))return;const l=`${t.first_name||""} ${t.last_name||""}`.trim()||"this student";if(!window.confirm(`Clear registration for ${l}? This deletes the student record and all related registration data.`))return;i.disabled=!0;const{error:f}=await j(t.id);if(f){i.disabled=!1,window.alert("Failed to clear registration: "+f.message);return}t=E.filter(c=>c.id!==t.id)[0]||null,S=[],k=[],g=1,n((t==null?void 0:t.id)||null,!t)})}function he(e){const n=document.getElementById("wizard-back-btn");n&&n.addEventListener("click",()=>{g>1&&(g--,e(t==null?void 0:t.id))}),document.querySelectorAll(".edit-step-btn").forEach(o=>{o.addEventListener("click",()=>{g=parseInt(o.dataset.step,10),e(t==null?void 0:t.id)})});const a=document.querySelector(".skip-step-btn");a&&a.addEventListener("click",()=>{g++,e(t==null?void 0:t.id)})}async function _e(e){const n=document.getElementById("photo-preview");if(!n||!e)return;const{url:a}=await V(e);a&&(n.innerHTML=`<img src="${a}" alt="Student photo" class="photo-preview" />`)}function xe(){const e=Array.from(document.querySelectorAll(".role-rank-select"));if(e.length===0)return;function n(){const a=new Set;e.forEach(i=>{const l=i.value;if(l){if(a.has(l)){i.value="";return}a.add(l)}});const o=new Set(e.map(i=>i.value).filter(Boolean));e.forEach(i=>{Array.from(i.options).forEach(l=>{if(!l.value){l.disabled=!1;return}l.disabled=l.value!==i.value&&o.has(l.value)})})}e.forEach(a=>{a.addEventListener("change",()=>{const o=document.getElementById("role-prefs-msg");o&&(o.textContent=""),n()})}),n()}function Ee(e,n){const a=document.getElementById("student-form");a&&a.addEventListener("submit",async c=>{var b;c.preventDefault();const r=document.getElementById("student-form-msg"),s=a.querySelector("button"),p={firstName:document.getElementById("reg-first-name").value.trim(),lastName:document.getElementById("reg-last-name").value.trim(),grade:document.getElementById("reg-grade").value.trim()},m=((b=document.getElementById("reg-student-email"))==null?void 0:b.value.trim())||"";if(!p.firstName||!p.lastName||!p.grade){r.textContent="First name, last name, and grade are required.",r.className="form-message error";return}if(s.disabled=!0,s.textContent="Saving…",t){const{data:y,error:u}=await P(t.id,{first_name:p.firstName,last_name:p.lastName,grade:p.grade,student_email:m||null},e);if(u){r.textContent="Error: "+u.message,r.className="form-message error",s.disabled=!1,s.textContent="Save & Continue";return}t={...t,...y},await I(e),g=2,n(t.id)}else{const{data:y,error:u}=await F({familyAccountId:e,firstName:p.firstName,lastName:p.lastName,grade:p.grade,studentEmail:m||void 0,createdByUserId:e});if(u){r.textContent="Error: "+u.message,r.className="form-message error",s.disabled=!1,s.textContent="Save & Continue";return}t=y,await I(e),g=2,n(t.id)}});const o=document.getElementById("parent-form");if(o){o.addEventListener("submit",async r=>{var x,$,w,B,R;r.preventDefault();const s=document.getElementById("parent-form-msg"),p=o.querySelector("button"),m={parent_first_name:document.getElementById("reg-parent-first").value.trim(),parent_last_name:document.getElementById("reg-parent-last").value.trim(),parent_email:document.getElementById("reg-parent-email").value.trim(),parent_phone:document.getElementById("reg-parent-phone").value.trim()};if(((x=document.getElementById("reg-show-parent2"))==null?void 0:x.checked)?(m.parent2_first_name=(($=document.getElementById("reg-parent2-first"))==null?void 0:$.value.trim())||null,m.parent2_last_name=((w=document.getElementById("reg-parent2-last"))==null?void 0:w.value.trim())||null,m.parent2_email=((B=document.getElementById("reg-parent2-email"))==null?void 0:B.value.trim())||null,m.parent2_phone=((R=document.getElementById("reg-parent2-phone"))==null?void 0:R.value.trim())||null):(m.parent2_first_name=null,m.parent2_last_name=null,m.parent2_email=null,m.parent2_phone=null),!m.parent_first_name||!m.parent_last_name||!m.parent_email||!m.parent_phone){s.textContent="Parent 1 fields are required.",s.className="form-message error";return}if(Q(m.parent_email)){s.textContent="Student email addresses cannot be used. Please use a parent or guardian email.",s.className="form-message error";return}p.disabled=!0,p.textContent="Saving…";const{data:y,error:u}=await P(t.id,m,e);if(u){s.textContent="Error: "+u.message,s.className="form-message error",p.disabled=!1,p.textContent="Save & Continue";return}t={...t,...y},await I(e),g=3,n(t.id)});const c=document.getElementById("reg-show-parent2");c&&c.addEventListener("change",()=>{const r=document.getElementById("parent2-fields");r&&(r.style.display=c.checked?"block":"none")})}const i=document.getElementById("song-form");i&&(i.querySelectorAll('input[name="sings-own"]').forEach(c=>{c.addEventListener("change",()=>{const r=document.getElementById("song-name-field");r&&(r.style.display=c.value==="yes"&&c.checked?"block":"none")})}),i.addEventListener("submit",async c=>{var w,B;c.preventDefault();const r=document.getElementById("song-form-msg"),s=i.querySelector('button[type="submit"]'),p=((w=i.querySelector('input[name="sings-own"]:checked'))==null?void 0:w.value)==="yes",m=((B=document.getElementById("reg-song-name"))==null?void 0:B.value.trim())||"",{valid:b,error:y}=K(p,m);if(!b){r.textContent=y,r.className="form-message error";return}s.disabled=!0,s.textContent="Saving…";const u={sings_own_disney_song:p,song_name:p?m:null},{data:x,error:$}=await P(t.id,u,e);if($){r.textContent="Error: "+$.message,r.className="form-message error",s.disabled=!1,s.textContent="Save & Continue";return}t={...t,...x},g=4,n(t.id)}));const l=document.getElementById("photo-form");l&&(l.addEventListener("submit",async c=>{c.preventDefault();const r=document.getElementById("photo-form-msg"),s=l.querySelector("button"),m=document.getElementById("reg-photo").files[0];if(!m){r.textContent="Please select a photo.",r.className="form-message error";return}s.disabled=!0,s.textContent="Uploading…",r.textContent="";try{const b=M(e,m.name),{error:y}=await O(b,m);if(y){r.textContent="Upload failed: "+(y.message||JSON.stringify(y)),r.className="form-message error",s.disabled=!1,s.textContent="Upload & Continue";return}const{data:u,error:x}=await P(t.id,{photo_storage_path:b},e);if(x){r.textContent="Error saving photo reference: "+(x.message||JSON.stringify(x)),r.className="form-message error",s.disabled=!1,s.textContent="Upload & Continue";return}t={...t,...u},await I(e),g++,n(t.id)}catch(b){r.textContent="Unexpected error: "+(b.message||String(b)),r.className="form-message error",s.disabled=!1,s.textContent="Upload & Continue"}}),t!=null&&t.photo_storage_path&&_e(t.photo_storage_path)),xe();const v=document.getElementById("save-role-prefs-btn");v&&v.addEventListener("click",async()=>{const c=document.getElementById("role-prefs-msg"),r=document.querySelectorAll(".role-rank-select"),s=[];r.forEach(y=>{const u=parseInt(y.value,10);u&&s.push({roleId:y.dataset.roleId,rank:u})});const p=ae(s,C);if(!p.valid){c&&(c.textContent=p.errors[0]||"Each rank can only be used once.",c.className="form-message error");return}v.disabled=!0,v.textContent="Saving…",c&&(c.textContent="");const{error:m}=await te(t.id,s,e);if(m){c&&(c.textContent="Error: "+m.message,c.className="form-message error"),v.disabled=!1,v.textContent="Save & Continue";return}const{error:b}=await ne(t.id);b&&console.warn("Vocal day auto-assign:",b.message),g++,n(t.id)});const f=document.getElementById("skip-role-prefs-btn");f&&f.addEventListener("click",()=>{g++,n(t.id)});const _=document.getElementById("contract-form");_&&_.addEventListener("submit",async c=>{c.preventDefault();const r=document.getElementById("contract-form-msg"),s=_.querySelector("button"),p=document.getElementById("reg-student-sig").value.trim(),m=document.getElementById("reg-parent-sig").value.trim(),{valid:b,errors:y}=W(p,m);if(!b){r.textContent=y.join(" "),r.className="form-message error";return}s.disabled=!0,s.textContent="Signing…";const{error:u}=await G({studentId:t.id,contractId:h.id,studentTypedSignature:p,parentTypedSignature:m,signedByUserId:e}),x=!!(u&&typeof u.message=="string"&&(u.message.includes("contract_acceptances_student_contract_idx")||u.message.toLowerCase().includes("duplicate key value")));if(u&&!x){r.textContent="Error: "+u.message,r.className="form-message error",s.disabled=!1,s.textContent="Sign & Complete Registration";return}const{data:$}=await z(t.id);S=$||[];const{emailError:w}=await I(e,{forceSendScheduleEmail:!0});if(w){r.textContent=`Registration completed, but schedule email failed to send. Please contact staff. (${w.message||"Email send error"})`,r.className="form-message error",s.disabled=!1,s.textContent="Signed (Email Retry Needed)";return}n(t.id)})}function Ae(){const e=document.createElement("div");e.className="page",e.innerHTML=`
  ...
  `;const{user:n}=U();if(!n)return e;const a=oe();async function o(i,l=!1){await ie(n.id,i,l);const v=document.getElementById("reg-student-selector"),f=document.getElementById("reg-checklist"),_=document.getElementById("reg-content");if(!_)return;v&&(v.innerHTML=de()),f&&(f.innerHTML=t?ce():"");const c=T(),r=c[g-1]||c[c.length-1];let s="";r==="Student Info"?s=pe():r==="Parent/Guardian"?s=ue():r==="Audition Song"?s=ge():r==="Photo"?s=fe():r==="Role Preferences"?s=ve():r==="Review & Sign"&&(s=be());const p=g>1?'<button id="wizard-back-btn" class="btn-ghost" style="margin-bottom:var(--space-md)">← Back</button>':"";_.innerHTML=`
      ${le()}
      ${p}
      ${s}
    `,ye(n.id,o),he(o),Ee(n.id,o)}return setTimeout(()=>o(a),0),e}export{Ae as renderFamilyRegistration};
