...
describe('hasRequiredStudentFields', () => {
  ...
  it('returns false when a field is whitespace only', () => {
    expect(hasRequiredStudentFields({ first_name: '  ', last_name: 'Doe', grade: '5' })).toBe(false);
  });

  it('returns true when all required fields present', () => {
    expect(hasRequiredStudentFields({ first_name: 'Jane', last_name: 'Doe', grade: '5' })).toBe(true);
  });
});
...
describe('hasValidAcceptance', () => {
  ...
  it('returns false for null activeContractId', () => {
    expect(hasValidAcceptance([{ contract_id: 'c1', student_typed_signature: 'A', parent_typed_signature: 'B' }], null)).toBe(false);
  });

  it('returns false when no matching acceptance', () => {
    const acceptances = [{ contract_id: 'c1', student_typed_signature: 'A', parent_typed_signature: 'B' }];
    expect(hasValidAcceptance(acceptances, 'c2')).toBe(false);
  });

  it('returns false when matching acceptance has empty student signature', () => {
    const acceptances = [{ contract_id: 'c1', student_typed_signature: '', parent_typed_signature: 'B' }];
    expect(hasValidAcceptance(acceptances, 'c1')).toBe(false);
  });

  it('returns false when matching acceptance has empty parent signature', () => {
    const acceptances = [{ contract_id: 'c1', student_typed_signature: 'A', parent_typed_signature: '' }];
    expect(hasValidAcceptance(acceptances, 'c1')).toBe(false);
  });
...
describe('evaluateRegistration', () => {
  const completeStudent = {
    ...
    last_name: 'Doe',
    grade: '5',
    photo_storage_path: 'photos/abc.jpg',
  };
  ...
  it('returns all three missing items when nothing is done', () => {
    const emptyStudent = {};
